...
class UserController {
  ...
  static async createUser(req, res) {
    const { firstname, lastname, email, hashPassword, salt, isActive } = req.validated.body;
  ...
  static async updateCurrentUser(req, res) {
    ...
    const { firstname, lastname, email, hashPassword, salt, isActive } = req.validated.body;
    const updatedUser = await UserService.updateUserById(userId, { firstname, lastname, email, hashPassword, salt, isActive });
    return res.status(200).json({
